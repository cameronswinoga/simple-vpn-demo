...
//
void ComputeUdpPseudoHeaderChecksumV4(char *pseudobuf, void *iphdr, UDP_HDR *udphdr, char *payload, int payloadlen)
{
    IPV4_HDR *v4hdr    = NULL;
...
//    for computing the pseudo-checksum. It is assumed this buffer is large enough.
//
void ComputeUdpPseudoHeaderChecksumV6(char *pseudobuf, void *iphdr, UDP_HDR *udphdr, char *payload, int payloadlen)
{
    IPV6_HDR *v6hdr      = NULL;
...
//
int PacketizeIpv4(WSABUF *Packets,
                  int *PacketCount,
                  char *PseudoBuffer,
                  struct addrinfo *src,
                  ...
                  int payloadlen)
{
    ...
    if ((Packets == NULL) || (PacketCount == NULL) || (src == NULL) || (dest == NULL) || (payload == NULL) || (*PacketCount < 1)) {
    ...
    iphdrlen = InitIpv4Header(Packets[0].buf, src->ai_addr, dest->ai_addr, DEFAULT_TTL, gProtocol, sizeof(UDP_HDR) + payloadlen);

    // Initialize the UDP header
    udphdrlen = InitUdpHeader(&Packets[0].buf[iphdrlen], src->ai_addr, dest->ai_addr, payloadlen);

    // Compute the UDP checksum
    ComputeUdpPseudoHeaderChecksumV4(PseudoBuffer, Packets[0].buf, (UDP_HDR *) &Packets[0].buf[iphdrlen], payload, payloadlen);
...
//
int PacketizeIpv6(WSABUF *Packets,
                  int *PacketCount,
                  char *PseudoBuffer,
                  struct addrinfo *src,
                  struct addrinfo *dest,
                  ...
                  int payloadlen)
{
    ...
    // Check the parameters
    if ((Packets == NULL) || (PacketCount == NULL) || (src == NULL) || (dest == NULL) || (payload == NULL))
        return SOCKET_ERROR;
    ...
    do {
        ...
        iphdrlen = InitIpv6Header(Packets[numpackets].buf, src->ai_addr, dest->ai_addr, DEFAULT_TTL,
                                  (fragment ? FRAGMENT_HEADER_PROTOCOL : gProtocol), datalen + plushdrs);

        // Build the fragmentation header if necessary
        if (fragment) {
            iphdrlen += InitIpv6FragmentHeader(&Packets[numpackets].buf[iphdrlen],
                                               offset,  // offset from start of packet
                                               gProtocol, fragid, lastfragment);
        }

        // The first fragment includes the UDP header, subsequent fragments don't
        if (numpackets == 0) {
            udphdrlen = InitUdpHeader(&Packets[numpackets].buf[iphdrlen], src->ai_addr, dest->ai_addr,
                                      originalpayload  // payloadlen
            );

            // Compute the checksum
            ComputeUdpPseudoHeaderChecksumV6(PseudoBuffer, Packets[numpackets].buf, (UDP_HDR *) &Packets[numpackets].buf[iphdrlen],
                                             payload, payloadlen);
        }
        else {
            udphdrlen = 0;
...
int _cdecl main(int argc, char **argv)
{
    ...
    if (gSender) {
        ...
        if (ressrc->ai_family == AF_INET) {
            rc = PacketizeIpv4(fragments, &fragmentcount, buffer, ressrc, resdest, payload, gSendSize);
        }
        else if (ressrc->ai_family == AF_INET6) {
            rc = PacketizeIpv6(fragments, &fragmentcount, buffer, ressrc, resdest, payload, gSendSize);
        }
        ...
        for (i = 0; i < (int) gSendCount; i++) {
            for (j = 0; j < fragmentcount; j++) {
                rc    = sendto(s, fragments[j].buf, fragments[j].len, 0, resdest->ai_addr, (int) resdest->ai_addrlen);
                bytes = rc;
                if (rc == SOCKET_ERROR) {
        ...
        rc = bind(s, ressrc->ai_addr, (int) ressrc->ai_addrlen);
        if (rc == SOCKET_ERROR) {
        ...
        while (1) {
            ...
            rc      = recvfrom(s, buffer, MAX_PACKET, 0, (SOCKADDR *) &safrom, &fromlen);
            if (rc == SOCKET_ERROR) {
